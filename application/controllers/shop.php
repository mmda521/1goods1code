...
class Shop extends CI_Controller {
    ...
    * */
    ...
    {
         $this->load->view('1goods1code/shop');
    ...
    * */
    public function ajax_data(){
        ...
        $product_SellerName = $this->input->get_post("product_SellerName");
        if(!empty($product_SellerName)){
            $condition['product_SellerName'] = $product_SellerName;
        }
		
        
        $product_ShopContactPerson = $this->input->get_post("product_ShopContactPerson");
        if(!empty($product_ShopContactPerson)){
            $condition['product_ShopContactPerson'] = $product_ShopContactPerson;
        }

        $total = $this->shop_model->count_num($condition);  	
        $page_string = $this->common_page->page_string($total, $limit, $page);
        $list = $this->shop_model->get_list($condition,$limit,$offset);
    ...
    * */
     public function add()
    {      
         $this->load->view('1goods1code/shop_add');
    }
    ...
    public function doadd(){
        ...
        $product_SellerName       = $this->input->get_post("product_SellerName");
		 if(!empty($product_SellerName)) {
            ...
            echo result_to_towf_new('1', 0, '商家名称不能为空', NULL);
            exit();
        }
        $product_ShopContactPerson   = $this->input->get_post("product_ShopContactPerson");
		 if(!empty($product_ShopContactPerson)) {
            ...
            echo result_to_towf_new('1', 0, '联系人不能为空', NULL);
            exit();
        }
		$product_ShopPhoneNo    = $this->input->get_post("product_ShopPhoneNo");
		if(!empty($product_ShopPhoneNo)) {
            $condition['product_ShopPhoneNo'] = $product_ShopPhoneNo;
        }else {
            echo result_to_towf_new('1', 0, '联系方式不能为空', NULL);
            exit();
        }
        $product_ShopEmail = $this->input->get_post("product_ShopEmail");
        $product_ShopAddTime = $this->input->get_post("product_ShopAddTime");
        //插入数据
        $data = array(
            'product_SellerName'      =>$product_SellerName,
            'product_ShopContactPerson'  =>$product_ShopContactPerson,
			'product_ShopPhoneNo'   =>$product_ShopPhoneNo,			
            'product_ShopEmail'=>$product_ShopEmail,
			'product_ShopAddTime'=>$product_ShopAddTime
        );
    ...
    * */
    ...
    {
        $product_ShopID = $this->input->get_post("product_ShopID");		
		$condition = array();
        $condition['product_ShopID'] = $product_ShopID;
		//PC::debug($GUID);
		$list['info'] = $this->shop_model->get_list_row($condition);  
     // PC::debug($list);	
	    $this->load->view('1goods1code/shop_edit', $list);
...
/**
     ...
     public function do_edit()
    {
       ...
       $product_SellerName       = $this->input->get_post("product_SellerName");
        ...
        $product_ShopEmail = $this->input->get_post("product_ShopEmail");
         //编辑数据
       $data = array(
	        'product_ShopID'      =>$product_ShopID,
            'product_SellerName'      =>$product_SellerName,
            'product_ShopContactPerson'  =>$product_ShopContactPerson,
			'product_ShopPhoneNo'   =>$product_ShopPhoneNo,			
            'product_ShopEmail'=>$product_ShopEmail
        );
       $this->shop_model->update($data,$product_ShopID);
	    echo result_to_towf_new('1', 1, 'success', null);
    }
    ...
    * */
    ...
    { 
         $product_ShopID = $this->input->get_post("product_ShopID");
		 //PC::debug($product_ShopID);
      if ($product_ShopID) {
            $this->shop_model->delete($product_ShopID);
