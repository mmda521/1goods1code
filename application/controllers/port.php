...
class Port extends CI_Controller {
    ...
    * */
    ...
    {
         $this->load->view('1goods1code/port');
    ...
    * */
    public function ajax_data(){
        ...
        $product_PortName = $this->input->get_post("product_PortName");
        if(!empty($product_PortName)){
            $condition['product_PortName'] = $product_PortName;
        }

        $total = $this->port_model->count_num($condition);  	
        $page_string = $this->common_page->page_string($total, $limit, $page);
        $list = $this->port_model->get_list($condition,$limit,$offset);
    ...
    * */
     public function add()
    {      
         $this->load->view('1goods1code/port_add');
    }
    ...
    * */
    public function doadd(){
        ...
        //插入数据
        $data = array(
            'product_PortName'      =>$product_PortName
        );
    ...
    * */
    ...
    {
        $product_PortID = $this->input->get_post("product_PortID");		
		$condition = array();
        $condition['product_PortID'] = $product_PortID;
		//PC::debug($GUID);
		$list['info'] = $this->port_model->get_list_row($condition);  
     // PC::debug($list);	
	    $this->load->view('1goods1code/port_edit', $list);
...
/**
    ...
    * */
     public function do_edit()
    {
       ...
       $product_PortName       = $this->input->get_post("product_PortName");
         ...
         //编辑数据
       $data = array(
	        'product_PortID'      =>$product_PortID,
            'product_PortName'      =>$product_PortName
        );
       $this->port_model->update($data,$product_PortID);
	    echo result_to_towf_new('1', 1, 'success', null);
    }
    ...
    * */
    ...
    { 
      ...
      if ($product_PortID) {
            $this->port_model->delete($product_PortID);
        }
		echo result_to_towf_new('1', 1, 'success', null);
    }
