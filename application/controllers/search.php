...
class Search extends CI_Controller {
    ...
    * */
	 ...
	 public function dosearch()
    {
	   ...
	   $total = $this->search_model->search_openid($openid);
	    //PC::debug($list);
	   ...
	   {	
        if($total['Error_count']<5)
		   ...
		   $this->search_model->update_openid($data,$openid); 
	   ...
	   else 
	   {
		   ...
		   $this->search_model->update_openid($data,$openid); 
		   $result = $this->search_model->get_search($fwid);
			  ...
			  {				
                ...
                'SH_mch_billno'      =>$var['mch_billno'],	
				 'WX_send_listid'      =>$var['send_listid']				
				  ...
				  $this->search_model->update($data,$fwid);
	   ...
	   {
		$subtract = $this->search_model->search_num($total['Error_time']);
		if($subtract['a']>=1)
		{
		 $data = array(
            'Error_time'          =>null,
		  ...
		  $this->search_model->update_openid($data,$openid); 
		   $total1 = $this->search_model->search_openid($openid);
		  if(empty($list))
	      {           
			 $total1['Error_count']++; 
             $data = array(
	   ...
	   {
		   $result1 = $this->search_model->get_search($fwid);
