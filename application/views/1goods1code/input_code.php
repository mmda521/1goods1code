...
<link href="<?php echo base_url();?>/webroot/OGOC/css/deling.css" rel="stylesheet">
	<script>
		function search(){
			var url="<?php echo site_url("search/dosearch");?>?inajax=1";
			var data_ = {
				'time':<?php echo time();?>,
				'action':'ajax_data',
				'fwid':$("#fwid").val(),
			...
			$.ajax({
				...
				success: function(msg){
					...
					if(msg.resultcode<0){
						...
						//BUI.Message.Alert(msg.resultinfo.errmsg,'error');
						window.location.href = "<?php echo site_url('search/search2');?>";
						return false ;
					} else {
						window.location.href = "<?php echo site_url('search/search1');?>?money="+money1+"&&num="+num;
